...
This is an "enlightened" version of the WP plugin "Mobile Sharebar", licensed under the GPLv3.
Original plugin code can be downloaded at https://wordpress.org/plugins/mobile-sharebar/
Original Author is "Maik Hebestadt on behalf of somutech.de" at http://somutech.de/

Changes remove external content mainly:
---------------------------------------
* removed iframes with external content of plugin author's website to fasten access of admin pages
* removed inclusion of external js and css (yandex, see below) to fasten access
...
function mobile_sharebar_add_menu() {
	add_options_page('mO ShareBar', 'mO ShareBar', 'manage_options', 'mO-sharebar', 'mobile_sharebar_main');
}
...
function mobile_sharebar_admin_styles(){
	wp_enqueue_style( 'sharebar-admin', plugins_url('css/style.css', __FILE__) );
	if(get_option('sharebar_use_customcss') != true){
		wp_enqueue_style( 'sharebar', plugins_url('css/sharebar.css', __FILE__) );
	}
	// Source: http://yandex.st/highlightjs/8.0/styles/default.min.css
	wp_enqueue_style( 'yandex', plugins_url('lib/yandex.min.css', __FILE__ ) );
}

function mobile_sharebar_admin_scripts(){
	// Source: http://yandex.st/highlightjs/8.0/highlight.min.js
	wp_enqueue_script( 'yandex', plugins_url('lib/yandex-highlight.min.js', __FILE__ ) );
}
...
function mobile_sharebar_main() {
	...
	if(get_locale() != 'de_DE'){
		...
		$string[13] = 'Display mO ShareBar automatically';
		...
		$string[17] = '\'mO ShareBar\'-Link @ Frontpage';
		...
		$string[37] = 'Use custom CSS<br/><small>(This will replace the mO-Sharebar CSS, <br/>but can be revoked at any time)<small>';
	}else{
		...
		$string[13] = 'Zeige mO ShareBar automatisch';
		...
		$string[17] = '\'mO ShareBar\'-Link auf der Hauptseite';
		...
		$string[37] = 'Eigenes CSS benutzen<br/><small>(Ersetzt das mO-Sharebar CSS. Kann jederzeit Rückgängig gemacht werden)</small>';
	}
...
#mbl-sharebar .sharebar-button.sharebar-google{}');
	...
	<div class="wrap" id="sharebar_admin">
		<h2>mO ShareBar <?php echo SHAREBAR_VERSION; ?></h2>
...
If you don't want to display the mO ShareBar automatically, you can use the following codes to embed it yourself:<br/><br/>
<b>Initialize</b><br/>
...
&lt;link rel="stylesheet" href="<?php echo plugins_url('css/sharebar.css', __FILE__); ?>" type="text/css" />
&lt;script type="text/javascript" src="<?php echo plugins_url('js/sharebar.js', __FILE__); ?>">&lt;/script>
</pre>
</div>
<script>
